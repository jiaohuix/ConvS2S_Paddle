...
class DistributedDynamicBatchSampler(BatchSampler):
    ''' 支持多卡训练的动态bsz采样器,与fairseq对齐。 10/2
        ...
        *3.v1的src与tgt基本上句长差不多，不符合实际
        4.v1的tokens数容易溢出
        修正v2 bug: 12/1
        1.配置里尽量用max_tokens=4000，max_sentences=None,若指定max_sentences会使得src、tgt基本上相等，不符合论文。
        2.按句长顺序排，然后用_get_batches_by_max_tokens得到的初始frozen_batches里面不做shuffle操作，而是固定住放入init，利于重复使用和复现。
        3.两处shuffle操作：一个是在排序前的shuffle，common_seed=1是全局的随机种子（之前用的是epoch）；第二处是每个epoch对
        frozen_batches的shuffle使用epoch+common_seed作为随机种子（之前用的是epoch），其中epoch从1开始；这两处改动也与论文对齐，
        使得每个batch的输出与源码对齐。
        4.添加了len，因为frozen_batches数固定,可以得到dataloader的len了
        5.除了sampler的改动：data在pad时，按照src的实际词数进行降序排序，最上面的样本pad最少，见data.py的batchify_train_dev,需要稳定排序法mergsort
    '''
    def __init__(self,
                 dataset,
                 mode='train',
