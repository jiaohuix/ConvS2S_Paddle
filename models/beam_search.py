    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if len(eos_mask.shape) < 2:  # index_sample只能用于2d，不能完全替代gather
                eos_mask, cands_to_ignore = eos_mask.reshape([-1, cand_size]), cands_to_ignore.reshape([-1, beam_size])
            ...
            # active_bbsz_idx = paddle.gather(cand_bbsz_idx, axis=1, index=active_hypos)
            # active_scores = paddle.gather(cand_scores, axis=1, index=active_hypos)
        ...
        for sent in range(len(finalized)):
            ...
            # _, sorted_scores_indices = torch.sort(scores, descending=True) # values,indices
